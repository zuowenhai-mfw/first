...
// import { NgTableParams } from 'ng-table';
        ...
        /***/ function(module, exports, __webpack_require__) {
            ...
            var NgTableParams = (function () {
                ...
                /*输入框输入页码，回车事件*/
                NgTableParams.prototype.jumpPageKeyUp = function (e,page,pages) {
                    var keycode = window.event ? e.keyCode :e.which;
                    if(keycode == 13) {
                        if(pages.length){
                            for(var i = 0; i < pages.length; i++){
                                if(pages[i].type == 'last'){
                                    if(page !== undefined && page <= pages[i].number && page > 0){
                                        $(".jumptext").blur()
                                        return this.parameters({
                                            'page': page
                                        })
                                    }else{
                                        if(page > pages[i].number){
                                            $(".jumptext").val(pages[i].number).blur()
                                            return this.parameters({
                                                'page': pages[i].number
                                            })
                                        }else{
                ...
                return {
                    ...
                    link: function (scope, element /*, attrs*/) {
                        ...
                        ngTableEventsChannel.onAfterReloadData(function (pubParams) {
                            ...
                            var stepHeight = $(".stepHeight").height();

                            //策略管理中剪掉插件的头部
                            if(stepHeight == null){
                                stepHeight = 0;
                            }else{
                                stepHeight = $(".stepHeight").height() + 65
                            }

                            var mainH = $("#content-main").height();//主体部分高度

                            var tableH = mainH - otherHeight - tableTop - stepHeight - 110;//table高度

                            $(".tableFixed").css("max-height", tableH);
        ...
        /***/ function(module, exports, __webpack_require__) {
            ...
            var html = "<div class=\"ng-cloak ng-table-pager\" ng-if=\"params.data.length\">\n    " +
                            "<div ng-if=\"params.settings().counts.length\" class=\"ng-table-counts btn-group pull-right\">\n  " +
                                "<span class=\"pager_font\">每页显示&nbsp;" +
                                    "<span class=\" dropup \" style=\"position: relative\">" +
                                        "<button data-toggle=\"dropdown\" class=\"btn btn-xs btn-white dropdown-toggle\">{{params.count(count)}} " +
                                            "<span class=\"fa fa-caret-up\"></span>" +
                                        "</button> " +
                                        "<ul class=\"dropdown-menu\" style=\"min-width: 50px\"> " +
                                            "<li ng-repeat=\"count in params.settings().counts\" ng-class=\"{\'active\':params.count()==count}\" ng-click=\"params.count(count)\">" +
                                                "<a>{{count}}</a>" +
                                            "</li>" +
                                        "</ul> " +
                                    "</span>&nbsp;条记录&nbsp;&nbsp;" +
                                    /*"<input-text></input-text>" +*/
                                    "<span class=\"form-inline\">第&nbsp;<input class=\" form-control jumptext\" type=\"text\" ng-model=\"jumpnumber\" ng-keyup='params.jumpPageKeyUp($event,jumpnumber,pages)'/>&nbsp;页</span>" +
                                    "&nbsp;&nbsp;总共 {{params.total()}} 条 " +
                                "</span>\n  " +
                            "</div>\n    " +
